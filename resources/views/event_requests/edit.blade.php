                ...
                <div class="col-md-4">
                    ...
                    <select name="primary_ticket_type_id" class="form-select" required>
                        ...
                        @foreach($ticketTypes as $tt)
                            <option value="{{ $tt->id }}" {{ (int)old('primary_ticket_type_id', $eventRequest->primary_ticket_type_id) === $tt->id ? 'selected' : '' }}>{{ $tt->name }} @if(!is_null($tt->price)) - EGP {{ number_format($tt->price,2) }} @endif</option>
                        @endforeach
                    </select>
